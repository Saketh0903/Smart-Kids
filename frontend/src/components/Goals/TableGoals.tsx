...
import {
  ...
  setRefetch,
  deleteItemsByIds,
} from '../../stores/goals/goalsSlice';
import { useAppDispatch, useAppSelector } from '../../stores/hooks';
import { useRouter } from 'next/router';
import { Field, Form, Formik } from 'formik';
import { DataGrid, GridColDef } from '@mui/x-data-grid';
import { loadColumns } from './configureGoalsCols';
...
const TableSampleGoals = ({
  ...
  useEffect(() => {
    if (goalsNotify.showNotification) {
      notify(goalsNotify.typeNotification, goalsNotify.textNotification);
    }
  }, [goalsNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/goals/goals-view/?id=${id}`);
  };

  const handleCreateEventAction = ({ start, end }: SlotInfo) => {
    router.push(
      `/goals/goals-new?dateRangeStart=${start.toISOString()}&dateRangeEnd=${end.toISOString()}`,
    );
  };
    ...
    <>
      ...
      {!showGrid && (
        <BigCalendar
          ...
          pathEdit={`/goals/goals-edit/?id=`}
          pathView={`/goals/goals-view/?id=`}
...
export default TableSampleGoals;

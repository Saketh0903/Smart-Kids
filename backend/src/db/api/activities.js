...
module.exports = class ActivitiesDBApi {
  static async create(data, options) {
    ...
    await activities.setParticipants(data.participants || [], {
      transaction,
    });
  ...
  static async findBy(where, options) {
    ...
    output.participants = await activities.getParticipants({
      transaction,
    });
  ...
  static async findAll(filter, options) {
    ...
    if (filter) {
      ...
      if (filter.name) {
        where = {
          ...where,
          [Op.and]: Utils.ilike('activities', 'name', filter.name),
        };
      }
  ...
  static async findAllAutocomplete(query, limit) {
    ...
    if (query) {
      where = {
        [Op.or]: [
          { ['id']: Utils.uuid(query) },
          Utils.ilike('activities', 'name', query),
        ],
      };
    }
